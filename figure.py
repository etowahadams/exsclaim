...
def detect_subfigures(fig_path: str):
    ...
    checkpoint = "object_detection_model.pt"
...
def detect_subfigure_labels(figure_path, subfigure_info):
        ...
        """
        model = resnet152()
        checkpoint = "text_recognition_model.pt"
        device = torch.device("cpu")
        model.to(device)
        model.load_state_dict(torch.load(checkpoint, map_location="cpu"))
        model.eval()
...
def classify_subfigures(figure_path, subfigure_labels, concate_img):
        ...
        checkpoint = "classifier_model.pt"
        device = torch.device("cpu")
        model.to(device)
        model.load_state_dict(torch.load(checkpoint, map_location="cpu"))
        model.eval()
        ...
        if len(subfigure_labels) > 0:
            ...
            subfigure_labels = process.label2yolobox(subfigure_labels, info_img, img_size, lrflip=False)
...
if __name__ == "__main__":
    figure = "7614755_F2.jpg"
    bounding_boxes = detect_subfigures(figure)
    show_image_with_bboxes(figure, bounding_boxes)
    # bounding_boxes = [[486, 9, 511, 33, '1.000'], [822, 9, 847, 33, '1.000'], [16, 9, 42, 32, '1.000'], [400, 644, 422, 669, '1.000'], [22, 641, 43, 668, '1.000'], [842, 642, 866, 666, '1.000'], [20, 239, 46, 262, '1.000'], [589, 241, 614, 265, '1.000'], [870, 251, 883, 271, '1.000'], [36, 611, 62, 641, '1.000'], [316, 270, 325, 295, '1.000'], [849, 242, 869, 285, '1.000'], [881, 238, 904, 281, '1.000'], [49, 556, 90, 582, '1.000'], [829, 271, 843, 292, '1.000'], [874, 227, 886, 251, '1.000'], [492, 269, 506, 282, '1.000'], [18, 179, 44, 203, '1.000'], [543, 418, 565, 444, '1.000'], [828, 828, 840, 838, '1.000'], [317, 300, 329, 319, '1.000'], [744, 609, 793, 626, '1.000']]
    subfigure_info, concate_img = detect_subfigure_labels(figure, bounding_boxes)
    # print(subfigure_info)
